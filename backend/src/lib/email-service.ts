...
const FROM_NAME = process.env.SENDGRID_FROM_NAME || 'GHL Workflow Debugger';
...
if (SENDGRID_API_KEY) {
  ...
  logger.warn('⚠️  SendGrid not configured - emails will be logged only');
}
...
export async function sendWorkflowFailureAlert(email: string, alert: WorkflowAlert): Promise<void> {
  ...
  const icon = alertType === 'error' ? '🚨' : alertType === 'warning' ? '⚠️' : 'ℹ️';
...
export async function testEmailConfiguration(email: string): Promise<boolean> { 
  try {
    await sendEmail({ to: email, subject: 'Test Email - GHL Workflow Debugger', html: '<html><body><h1>✅ Test Successful</h1><p>Your SendGrid email configuration is working correctly!</p></body></html>' });
    return true;
  } catch {
    return false;
  }
